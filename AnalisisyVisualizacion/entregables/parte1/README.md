...
- **2a) Densidad conjunta:** 3 variables numéricas (`salary_monthly_NETO`, `profile_years_experience`, `profile_age`) y 2 categóricas (`work_seniority`, `profile_gender`). Se incluyen histogramas marginales con el rango de edad acotado al tramo laboralmente plausible `[15, 80]` años, matriz de correlación de Pearson (coherente con la clase 03) con el eje Y invertido para que la diagonal de 1 quede ascendente, y distribución de las categóricas mostrando los tres grupos analíticos de género.
